...
| Active-page accessibility snapshots, screenshots and browser tool results | Answer a browser tool a local agent calls through `vibewaiting mcp`. | Returned through the loopback native companion to that MCP server. Vibewaiting does not host a copy; the calling agent may retain tool input/output under its own transcript policy. |
