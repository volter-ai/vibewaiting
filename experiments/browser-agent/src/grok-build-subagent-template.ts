...
/** Pinned from xai-grok-agent/templates/subagent_prompt.md at 9684fa3c. */
export const GROK_BUILD_SUBAGENT_TEMPLATE = `You are a Grok Build subagent — a focused worker delegated a specific task.
...
export function renderGrokBuildTemplate(template: string, values: Values): string {
  const conditional = /\$\{%(-?)\s*(if\s+([^%]+)|else|endif)\s*%\}/gu;
  type Frame = { parent: boolean; condition: boolean; active: boolean };
  const stack: Frame[] = [];
  let active = true;
  let cursor = 0;
  let output = "";
  for (const match of template.matchAll(conditional)) {
    if (active) output += template.slice(cursor, match.index);
    if (match[1] === "-" && active) output = output.replace(/\s+$/u, "");
    const directive = match[2]!;
    if (directive.startsWith("if ")) {
      const condition = evaluateExpression(match[3]!, values);
      stack.push({ parent: active, condition, active: active && condition });
      active = active && condition;
    } else if (directive === "else") {
      const frame = stack.at(-1);
      if (!frame) throw new Error("template else without if");
      frame.active = frame.parent && !frame.condition;
      active = frame.active;
    } else {
      if (!stack.pop()) throw new Error("template endif without if");
      active = stack.at(-1)?.active ?? true;
    }
    cursor = match.index! + match[0].length;
  }
  if (stack.length) throw new Error("template if without endif");
  if (active) output += template.slice(cursor);
  return output.replace(/\$\{\{\s*([^}]+?)\s*\}\}/gu, (_match, path: string) => String(resolvePath(values, path.trim()) ?? ""));
}

function evaluateExpression(expression: string, values: Values): boolean {
  return expression.trim().split(/\s+and\s+/u).every((term) => {
    const defined = /^(.+?)\s+is\s+defined$/u.exec(term);
    if (defined) return resolvePath(values, defined[1]!.trim()) !== undefined;
    const equality = /^(.+?)\s*==\s*"([^"]*)"$/u.exec(term);
    if (equality) return resolvePath(values, equality[1]!.trim()) === equality[2];
    return Boolean(resolvePath(values, term.trim()));
  });
}

function resolvePath(values: Values, path: string): unknown {
  let value: unknown = values;
  for (const part of path.split(".")) {
    if (!value || typeof value !== "object") return;
    value = (value as Values)[part];
  }
  return value;
}
