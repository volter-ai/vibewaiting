...
export function parseQuestions(rawQuestions: unknown[]): GrokQuestion[] {
  ...
  return rawQuestions.map((raw) => {
    ...
    if (typeof value.question !== "string" || !value.question) throw new Error("Question text is required.");
    if (!Array.isArray(value.options) || value.options.length === 0) throw new Error("Question options are required.");
    const question: GrokQuestion = {
      ...
      options: value.options.map((rawOption) => {
        ...
        if (typeof option.label !== "string" || !option.label || typeof option.description !== "string") {
      ...
      ...(value.multi_select === true || value.multiSelect === true ? { multi_select: true } : {}),
