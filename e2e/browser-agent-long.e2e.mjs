...
test("strictly replays an arbitrary long native Grok Build session", async ({ page, request }) => {
  ...
  try {
    await expect(page.getByText(`${profile.modelRequests} native model requests matched with zero drift`, { exact: false }))
      .toBeVisible({ timeout: 50_000 });
  } catch (error) {
    console.error(`Browser trajectory at failure:\n${await page.locator("#trajectory").innerText()}`);
    throw error;
  }
  await expect(page.locator("#agent-state")).toHaveText("Complete");
});
