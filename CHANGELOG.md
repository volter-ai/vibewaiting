...
- An agent drives the shared tab through `vibewaiting mcp`, a stdio MCP server its
  ...
  AlmostCDP 0.1.30 (or Chrome's debugger on a page that forbids eval), and leaves out
...
- Browser permissions in the messenger, after Claude in Chrome's per-site permissions.
  ...
  asks unless the person allowed the tab's current origin for the agent's task (ended
  by the task, the tab closing, or 15 minutes unused); reading never asks. Cards offer
  Deny, Allow once and Allow on <origin> for this task; scripts, typing into password,
  one-time-code, card or file fields (or fields that were one on the document), raw
  input whose target cannot be identified, and pages without a real origin offer only
  Allow once, every time. `browser.script` runs its source as JavaScript in the page,
  with the page's own power; no agent code runs in the extension. Input is sent only
  to the document that was checked, with no navigation in flight. Cards name what
  actually happens (the key Playwright sends, a select's options, a drag's drop target,
  a script's full source and arguments) and, on the in-page path, say the page
  described it. Allow buttons arm after a second of visibility and stillness and half
  a second of the pointer resting on them. Deny, no
  ...
  side (Supercode browser-playwright 0.2.1).
